...
const api = (baseUrl) => {
    ...
    function loadingButton() {
        let content = document.createElement('span'), icon = document.createElement('span'), span = document.createElement('span');
    ...
    function nowLoading(formElement) {
        let tmp = formElement.querySelector('[type="submit"]'), tmpId = 'btn_' + Date.now();
        submitButton.push({
            id: tmpId,
    ...
    function stopLoading() {
        submitButton.map(button => {
            let tmp = document.querySelector(`[data-loading="${button.id}"]`);
            if (tmp) {
                tmp.disabled = false;
     ...
     */
    async function create(endpoint, data, method = 'post') {
        let response;
     ...
     */
    async function submit(formElement, afterSubmit, onError) {
                ...
                case 'POST': {
                    ...
                    try {
                        ...
                        if (onError)
                            ...
                            alert(`Falha ao enviar para ${endpoint}: ${error}`);
                    }
                    break;
                }
                default:
